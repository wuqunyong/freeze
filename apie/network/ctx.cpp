...
void Ctx::initMysqlConnector(LoadConfig<Mysql_MysqlConfig>& mysqlConfig)
{
	...
	PIE_LOG(PIE_NOTICE, "startup|mysql:{}|{}|{}", host.c_str(), db.c_str(), port);
}
...
void Ctx::init(const std::string& configFile)
{
	...
	try {
		...
		PIE_LOG(PIE_NOTICE, "startup|config:{}", configFile.c_str());
		...
		PIE_LOG(PIE_NOTICE, "startup|ioThreads: {}", ioThreads);
...
void Ctx::waitForShutdown()
{
	...
	while (true)
	{
		...
		if (prtGet != NULL)
		{
			...
			if (iResult == 0)
			{
				PIE_LOG(PIE_NOTICE, "startup|Aborting nicely");
				break;
			}
...
void Ctx::resetYamlNode(YAML::Node node)
{
	...
	if (ptrConfig == nullptr)
	{
		PIE_LOG(PIE_NOTICE, "Exception|reload|loadConfigs error");
		return;
	}
