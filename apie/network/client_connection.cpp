...
void apie::ClientConnection::handleSend(const char *data, size_t size)
{
	if (NULL != this->bev)
	{
		...
		if (rc != 0)
		{
			...
			PIE_LOG(PIE_ERROR,"Exception|ClientConnection|handleSend Error:{}|{}", rc, sData);
		}
		else
		{
			//std::stringstream ss;
...
std::shared_ptr<apie::ClientConnection> apie::ClientConnection::createClient(uint32_t threadId, struct event_base *base, DialParameters* ptrDial)
{
	...
	do
	{
		...
		if (rc != 0)
		{
			PIE_LOG(PIE_WARNING, "Exception|processActiveConnect|setsockopt|TCP_NODELAY:{}|{}:{}", rc, ip, iPort);
		}
		//assert(rc == 0);
		...
		if (rc != 0)
		{
			PIE_LOG(PIE_WARNING, "Exception|processActiveConnect|setsockopt|SO_KEEPALIVE:{}|{}:{}", rc, ip, iPort);
		}
		//assert(rc == 0);
