...
namespace apie {
	...
	{
		/* Log the stack trace */
		pieFmtLog("startup/startup", PIE_CYCLE_DAY, PIE_NOTICE, "------ STACK TRACE BEGIN ------");
		std::string traceStr = generateBackTrace();
		pieFmtLog("startup/startup", PIE_CYCLE_DAY, PIE_NOTICE, "sigaction: Got signal {}|{}|bt:{}", sig, strsignal(sig), traceStr.c_str());
		pieFmtLog("startup/startup", PIE_CYCLE_DAY, PIE_NOTICE, "------ STACK TRACE END ------");
